...
- (void)sms:(NSArray *)recipients :(NSString *)content :(WXModuleCallback)callback
{
    ...
    if ([MFMessageComposeViewController canSendText]) {
        ...
        [weexInstance.viewController presentViewController:messageController animated:YES completion:nil];
    }else{
        if (callback) {
...
- (void)contacts:(WXModuleCallback)callback
{
    ...
    ABAddressBookRequestAccessWithCompletion(addressBook, ^(bool granted, CFErrorRef error) {
        if (granted) {
            ...
            [weexInstance.viewController presentViewController:abVc animated:YES completion:nil];
